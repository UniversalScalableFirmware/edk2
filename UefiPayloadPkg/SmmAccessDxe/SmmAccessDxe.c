  ...
  )
{
  if ((mSmmAccess.SmmRegionState & EFI_SMRAM_LOCKED) != 0) {
    ...
    //
    DEBUG ((EFI_D_INFO, "Cannot open the locked SMRAM Region\n"));
  ...
  )
{
  if ((mSmmAccess.SmmRegionState & EFI_SMRAM_LOCKED) != 0) {
    ...
    //
    DEBUG ((EFI_D_INFO, "Cannot close the locked SMRAM Region\n"));
    return EFI_DEVICE_ERROR;
  }
  ...
  )
{
  if (mSmmAccess.SmmAccess.OpenState) {
    DEBUG ((EFI_D_INFO, "Cannot lock SMRAM when it is still open\n"));
    return EFI_DEVICE_ERROR;
  }
  ...
  )
{
  ...
  DEBUG ((EFI_D_INFO, "NumberOfSmmReservedRegions = 0x%x\n", SmmRegionNum));
